...
- (instancetype)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) _manager = [[XHGameManager alloc] init];
    return self;
}
...
- (void)loadBoardWithGrid:(XHGrid *)grid {
    ...
    UIImage *image = [XHGridView gridImageWithGrid:grid];
...
- (void)commitTranslation:(CGPoint)translation {
    ...
    if (absX > absY * VALID_SWIPE_DIRECTION_THRESHOLD) {
        translation.x < 0 ? [_manager moveToDirection:XHDirectionLeft] :
        [_manager moveToDirection:XHDirectionRight];
    } else if (absY > absX * VALID_SWIPE_DIRECTION_THRESHOLD) {
        translation.y < 0 ? [_manager moveToDirection:XHDirectionUp] :
        [_manager moveToDirection:XHDirectionDown];
    }
